...
function resetPokemonPositions() {
  // Define posições aleatórias para cada Pokémon
  charmander.style.right = `${Math.random() * 800}px`;
  charmander.style.top = `${Math.random() * 600}px`;
  
  zubat.style.right = `${Math.random() * 800}px`;
  zubat.style.top = `${Math.random() * 600}px`;
  
  pikaxu.style.right = `${Math.random() * 800}px`;
  pikaxu.style.top = `${Math.random() * 600}px`;
}

restart.addEventListener('click', () => {
  window.location.reload();
  restart.style.display = 'none';
  resetPokemonPositions(); // Mover Pokémon para novas posições
});
  ...
  switch (event.code) {
    case 'ArrowLeft':
      if (getRightPosition() < 770) {
        ash.style.right = `${getRightPosition() + 16}px`; // Aumentar de 8 para 16
    ...
    case 'ArrowDown':
      if (getTopPosition() < 625) {
        ash.style.top = `${getTopPosition() + 16}px`; // Aumentar de 8 para 16
...
// Chama a função para definir posições iniciais dos Pokémon
